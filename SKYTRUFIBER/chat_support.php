...
.media-video{max-width:220px;border-radius:12px;margin-top:6px}
...
/* Preview Modal */
#previewModal{position:fixed;inset:0;background:rgba(0,0,0,.6);display:none;
...
<div class="chat-wrap">
  ...
  <div class="chat-header">
    ...
    <div>
      ...
      <div class="head-sub">Connected</div>
    </div>
  </div>
...
<div id="previewModal">
  <div id="previewBox">
    ...
    <div id="previewContent" style="margin:12px 0;"></div>
    <button onclick="confirmSend(true)">Send</button>
    <button onclick="confirmSend(false)" style="background:#bbb;color:#000;">Cancel</button>
  </div>
</div>
...
fileEl.addEventListener('change',()=>{
  ...
  if(['jpg','jpeg','png','gif','webp'].includes(ext)){
    ...
    img.style.maxWidth="100%"; preview.appendChild(img);
  } else {
    const v=document.createElement('video');
    v.src=URL.createObjectURL(selectedFile);
    v.controls=true; v.style.maxWidth="100%"; preview.appendChild(v);
  }

  document.getElementById("previewModal").style.display="flex";
});

function confirmSend(ok){

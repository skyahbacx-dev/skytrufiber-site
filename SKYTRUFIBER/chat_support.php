...
.chat-wrap{
  ...
  box-shadow:var(--shadow);display:flex;flex-direction:column;overflow:hidden;border:var(--border);
}
...
.avatar{width:30px;height:30px;background:#ddd;border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:700}

.bubble{max-width:70%;padding:10px 12px;border-radius:18px;box-shadow:0 3px 8px rgba(0,0,0,.06);font-size:14px;white-space:pre-wrap}
...
.input-bar input{flex:1;padding:12px;border:1px solid #ddd;border-radius:999px;font-size:14px;}
...
#previewModal{position:fixed;inset:0;background:rgba(0,0,0,.6);display:none;align-items:center;justify-content:center;z-index:9999}
#previewBox{background:#fff;padding:16px;border-radius:14px;width:90%;max-width:330px;text-align:center}
</style>
</head>
...
function renderRow(m){
  ...
  if(m.media_path){
    if(m.media_type==="image"){
      ...
      video.src="../"+m.media_path; video.controls=true; video.className="media-video";
...
function sendMessage(){
  ...
  fd.append("sender_type","client");
  fd.append("message",msg);
  fd.append("username",USERNAME);
  if(selectedFile) fd.append("file",selectedFile);

  fetch("save_chat_client.php",{method:"POST",body:fd})
    .then(r=>r.json())
    .then(()=>{
      inputEl.value="";
      selectedFile=null;

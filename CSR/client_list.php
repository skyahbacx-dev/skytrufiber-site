...
if (!$csrUser) { http_response_code(401); exit("Unauthorized"); }
...
while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    ...
    if ($assigned === null) {
        $button = "<button class='assign-btn assign' onclick='openAssignModal($id, \"$name\")'>+</button>";
    }
    else if ($assigned === $csrUser) {
        $button = "<button class='assign-btn unassign' onclick='openUnassignModal($id, \"$name\")'>−</button>";
    }
    else {

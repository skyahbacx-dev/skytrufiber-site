<?php
/**
 * CSR Dashboard — SkyTruFiber
 * - Sidebar (collapsible, does NOT auto-open when clicking tabs)
 * - Tabs: All Clients | My Clients | Reminders | Survey | Edit Profile
 * - Client list shows email (pulled from users table by full_name)
 * - Aesthetic chat bubbles with timestamps
 * - Assign / Unassign protection
 * - Reminders preview banners (1 week / 3 days before due; shows “Sent” if present in reminders table)
 */
...
if (isset($_GET['ajax'])) {
  ...
  // ── Reminders panel (PREVIEW ONLY)
  if ($_GET['ajax'] === 'reminders') {
    $search = trim($_GET['q'] ?? '');
    $rows = [];

    // list of users with email and date_installed to compute cycles
    ...
    foreach ($u as $usr) {
      ...
      // compute the upcoming due date (same day as install, current or next month)
      ...
      if ((int)$today->format('d') > (int)$due->format('d')) {
        $due = (clone $base)->modify('first day of next month')->setDate(
          (int)$base->modify('first day of next month')->format('Y'),
          (int)$base->format('m'),
          min($dueDay, 28)
        );
      }
      ...
      // check reminders table if already sent for this cycle
...
.client-item{
  ...
  padding:12px 12px;margin:10px;border:1px solid var(--line);border-radius:10px;cursor:pointer;
...
/* Switch tab WITHOUT touching sidebar state */
